...
-- this below is using another method of jaoin give the same result
SELECT 
...
-- if you want to return only to value better to use IF like below
SELECT first_name,
	last_name,
    IFNULL(COUNT(rating), 0) AS 'COUNT',
    IFNULL(MIN(rating), 0) AS 'MIN',
...
-- Order doesn't matter for the below queries all are same
SELECT 
    title,
    rating,
